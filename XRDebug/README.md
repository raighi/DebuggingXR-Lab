...
Explication du fonctionnement de la partie events : 
- Le TimeController.Update() fait avancer le temps, le TimeMode notifie ses abonnés via OnTimeChanged?.Invoke(t), ce qui permet ensuite au contrôleur du système planétaire de calculer les nouvelles posiitions via le service ephemeris. Enfin la vue des planètes est mise à jour, et les changements sont effectifs dans la scène 3D.

J'ai corrigé et modifié l'UI pour la rendre plus ergonomique, mais je n'ai pas réussi à identifier le problème de manque d'interactivité de l'UI.

J'ai ajusté la taille des planètes, modifié l'orbite renderer. J'ai de nouveau tourné mon système pour qu'il soit vaguement aligné avec les orbites affichées mais un décalage en profondeur subsiste.

Je dois modifier mon script pour que le mode sélection/focus soit amélioré.

Pour les interactions le flux serait : 
Input System lit les boutons et récupère si besoin les actions, Input Action Manager récupère les actions, XR Ray interactor utilise ces actions afin de cliquer. Tracked Device Graphic Raycaster devrait interagir avec les boutons de l'UI. 

Système analogue pour le game object tirable. Celui-ci est tirable mais je n'ai pas bien configuré le solar system pour qu'il se déplace.
